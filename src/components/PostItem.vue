        ...
        <header class="text-center mb-8">
          ...
          <p class="text-grey-dark leading-normal">
            <span v-if="post.author">by
              <g-link
                :to="`${post.author.path}/`"
                class="text-grey-darker capitalize no-underline border-b border-transparent hover:border-grey transition-border-color"
                v-if="post.author"
              >{{ titleCase(post.author.title) }}</g-link>&nbsp;
            </span>
            <span v-if="post.tags && post.tags.length > 0">in
              <g-link
                :to="`${post.tags[0].path}/`"
                class="text-grey-darker capitalize no-underline border-b border-transparent hover:border-grey transition-border-color"
